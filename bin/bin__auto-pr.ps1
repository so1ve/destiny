...
hub diff --name-only | ForEach-Object {
    ...
    # if (!([System.IO.File]::Exists($versions_folder_path))) {
    #     [System.IO.File]::Create($versions_folder_path)
    # }
    # [System.IO.File]::WriteAllLines("$versions_folder_path", $pretty_json)
    Write-Output $pretty_json > $versions_folder_path
